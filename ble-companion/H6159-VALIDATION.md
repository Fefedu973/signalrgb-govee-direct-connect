...
## Luminosité zéro : retour utilisateur ultérieur

L'utilisateur a ensuite signalé que le réglage de luminosité0% laisse cette bande éclairée. La validation des niveaux40/60/80% ne couvrait pas ce cas. Le profil H6159 déclare donc `black_at_zero_brightness=True` : pendant le contrôle, une demande de0% doit produire du RGB noir `[0,0,0]` au lieu de compter sur le seul octet de luminosité04 égal à0. Le constructeur du paquet04 et les octets du snapshot restent inchangés ; la session applique cette politique de rendu propre au modèle.

L'utilisateur a ensuite confirmé que la bande reste éclairée même avec RGB `[0,0,0]`, alors que le pont rapporte bien cette couleur envoyée et la session active. **Le remplacement par du noir seul est donc insuffisant sur cette bande.** Le profil ajoute `power_off_on_black=True` : un noir effectif, issu du Canvas ou de la luminosité0%, doit produire une commande explicite power-off. Une couleur non noire peut rétablir l'alimentation seulement si la session était elle-même responsable de cette coupure ; une extinction externe conserve sa signification.

Le correctif a été installé sans redémarrer SignalRGB. Le19septembre2026 à18:51:54UTC, avec luminosité globale0% et luminosité propre à la bande100%, la session réelle a confirmé `state=blackout`, `ready=true`, `power=0`, `blackout_owned=true`, sans erreur. La commande d'extinction est suivie d'une lecture AA01 ; il ne s'agit pas seulement d'un paquet envoyé. La vérification du retour à une couleur est consignée séparément après manipulation du réglage global.

Les92tests Python et23tests du client générique passent. Ils couvrent notamment noir Canvas à luminosité propre100%, absence de double atténuation, extinction possédée par la session, reprise avec couleur préparée avantON, absence de flashON initial, respect d'unOFF externe, reconnexion et arrêt final noir. Les profils qui ne déclarent pas ces comportements, notamment H6008, gardent les deux valeurs par défaut `False`. Aucun nouvel encodage de couleur n'est inventé : les classes H6159 `SubModeColor` et `ColorFragment` de l'APK sérialisent RGB directement, sans substitution noire observée dans ces classes.
...
L'APK Govee Home7.6.21, classe `com.govee.h6159.ble.SubModeColor`, écrit `[02,R,G,B,bool,R2,G2,B2]` et relit ces mêmes champs. Le flag01 observé n'est donc pas du padding à supprimer. Dans `ColorFragment.J0()`, une sélection RGB place le triplet primaire choisi et le flag àfalse. `ColorFragment.K0()` suit un chemin blanc distinct : primaire `ColorUtils.toWhite()`, flagtrue et second triplet choisi. Cela établit la distinction de ces deux chemins dans l'application, sans démontrer une conversion précise en température de couleur ou leur traitement électrique interne. Les écritures RGB simples ont produit flag00 ; la restauration du mode original avec flag01 a été confirmée par relecture immédiate.
